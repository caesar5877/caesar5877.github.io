...
下面给你一个 **“任何系统都通用、面试官最喜欢、最快清晰锁定 Functional Requirements 的提问框架”**。

这是一个 **通用提问清单（General Discovery Questions）**，无论是 ride-sharing、Instagram、payment system、rate limiter、collaboration doc、food delivery，都可以直接使用。

我把这些问题整理成 **5 大类、15 个黄金问题**，足够让你迅速摸透面试官的预期，同时显得你非常 senior。

---

# ✅ **系统设计面试：5 大类、15 个万能提问（Functional Requirements Discovery）**

# **(A) 核心用户与目标（必问）**

1. **Who are the primary users?**
   → 用户角色不同，功能必然不同（乘客 vs 司机、商家 vs 顾客、内部 vs 外部 API）

2. **What is the single most important outcome the system must achieve?**
   → Surface 最关键 FR（如 real-time match、secure payment、low-latency feed）
...
7. **What output do we return, and to whom?**
   → 决定 API responses & consumer needs

8. **How fresh or real-time must the data be?**
   → Functional requirement，但会影响 NFR（latency, consistency）

9. **How large is the data we store or process?**
   → 判断是否需要 sharding、partitioning、cache、streaming
...
10. **What is in scope and what is explicitly out of scope?**
    → 这是 Staff-level question；避免 over-engineering。

11. **Should we handle failures or retries on behalf of the client?**
    → FR 决定：auto retry？manual retry？idempotent API？

12. **Do we need to support history, auditing, or versioning?**
    → 时间线数据（timeline）、版本控制（docs）、transaction history（payment）

---

# **(E) 成功标准与系统目标（终局要求）**

13. **What defines a successful operation from the user’s perspective?**
    → e.g., "status updates within 1 sec" 或 "search results must be relevant"

14. **Are there SLAs for specific workflows?**
    → 这是 “Functional requirement disguised as NFR”

15. **What are the edge cases the system must explicitly handle?**
    → 系统边界、异常路径、并发冲突，用户不熟练问不出这个问题。
...
> – what core workflows they need,
> – what inputs/outputs the system handles,
> – what’s in scope vs out of scope,
> – and any success criteria or SLAs we should meet?”
...
> “Do we need to support both passengers and drivers?”
> “Should I focus on matching, realtime location, or the full lifecycle including payment?”
...
# 2. Ride-Sharing 系统的所有关键 NFR（附推导路径）
...
存储（Redis、Aurora、DynamoDB）必须水平扩展。

---

## **③ Availability（高可用）**

**来源：这是一个 mission-critical 产品**
...
* 订单状态必须强一致（Booked → Accepted → Ongoing → Completed）
* 定价与位置使用 **eventual consistency** 是可接受的
* 支付必须 **exactly-once**

不同模块允许不同一致性级别 → 属于高阶设计能力。

---

## **⑤ Reliability（可靠性）**

**来源：行程不能乱、不能 duplicated ride、不能 charge 两次**

包括：

* Idempotent API（防重复请求）
* Retry + backoff
* Circuit breaker（避免下游过载）
* Queue durability（匹配队列）
...
* Metrics（司机数量、匹配成功率、延迟）
* Distributed tracing（如 OpenTelemetry）
* Logging（行程、匹配流程）

---

## **⑩ Cost Efficiency（成本有效性）**
...
> **“延扩可依安观隐”**
> （读音：yán kuò kě yī ān guān yǐn）

对应：

1. **延** — 延迟（Latency）
2. **扩** — 扩展性（Scalability）
3. **可** — 可用性（Availability）
4. **依** — 一致性（Consistency）
5. **安** — 安全（Security）
6. **观** — 可观测性（Observability）
7. **隐** — 隐私（Privacy）

这是业内最好记、最全、最不尬的 NFR 记忆方法。
...
✅ **Ride-Sharing 系统一张图的总结（系统设计面试可直接讲）**
✅ **把 NFR 绑定到架构图（考官最喜欢）**
✅ **如何在面试中自然地问 NFR clarifying questions**
✅ **如何用“黄金问题框架”演示 reasoning 能力**
